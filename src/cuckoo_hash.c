...
static inline void compute_hash(const void *key, size_t key_len,
            ...
            size_t max_depth, uint32_t offset, int phase)
{
  uint32_t mask = (1U << hash->power) - 1;
  size_t depth;
  for (depth = 0; depth < max_depth * phase; ++depth)
    {
      if (offset-- == 0)
